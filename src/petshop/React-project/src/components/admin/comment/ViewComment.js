        ...
        display_Commentdata = viewComment.map((item) => {
            return (
                <tr key={item.id}>
                    ...
                    <td><img src={`${config.BASE_URL}/${item.product.image}`} width="50px" alt={item.name} /></td>
                    <td>
                        <button type="button" onClick={(e) => deleteComment(e, item.id)} className="btn btn-danger btn-sm">Xóa</button>
                    </td>
                </tr>
            )
        });
